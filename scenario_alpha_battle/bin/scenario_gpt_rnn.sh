...
tuning_set=\
"768 1 0.01 512 80 1 64000"
while IFS=' ' read -r hidden_size num_layers max_lr max_len batch_size accumulate_grad_batches max_steps
do 
    export logger_name="gptrnn--$hidden_size-$num_layers-$max_lr-$max_len-$batch_size-$accumulate_grad_batches-$max_steps"
    python -m ptls.pl_train_module \
      logger_name=${logger_name} \
      trainer.gpus=[${device}] \
      pl_module.max_lr=${max_lr} \
      pl_module.seq_encoder.hidden_size=${hidden_size} \
      pl_module.seq_encoder.num_layers=${num_layers} \
      ...
      model_path="models/gptrnn__$logger_name.p" \
    --config-dir conf --config-name gpt_rnn_params
    ...
    do 
      python -m ptls.pl_inference \
        ...
        pl_module.seq_encoder.num_layers=${num_layers} \
        model_path="models/gptrnn__$logger_name.p" \
        embed_file_name="emb_gptrnn_${inference_pooling_strategy}__${logger_name}" \
      --config-dir conf --config-name gpt_rnn_params
    done
done <<< $tuning_set
