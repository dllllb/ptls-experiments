...
def init_logger(log_file, level=logging.INFO, mode="w"):
    logging.basicConfig(
        level=level, format="%(levelname)s:%(funcName)s(%(relativeCreated)d): %(message)s",
        handlers=[logging.StreamHandler(), logging.FileHandler(log_file, mode=mode)])
...
def main(args):
    ...
    if not os.path.exists(inp_file) or os.path.exists(target_file):
        err_msg = "output" if os.path.exists(inp_file) else "input"
        sys.stderr.write(f"Check {err_msg} files in [{os.path.dirname(inp_file)}].\n")
        sys.exit(1)

    init_logger(f"{inp_file}.log" if args.log_file is None else os.path.abspath(args.log_file))
    logger.info(f"numexpr (max threads) = {numexpr_max_threads}.")
    ...
    # group = data[[ID_COL, DATE_COL, FVAL_COL]].groupby(by=ID_COL, as_index=False, group_keys=False)
    # data[TARGET_VAR] = group.apply(forward_window, event_time=DATE_COL, target=FVAL_COL, dt=args.dt)
...
def make_sample(args):
    ...
    with gzip.open(inp_file, "rt") as fin, gzip.open(out_file, "wt") as fout:
        header = next(fin)
        try:
            uid_index = header.strip("\n").split(",").index(args.sample_id)
        except:
            sys.stderr.write(f"ERROR: {args.sample_id} is not a valid column name!\n")
            sys.exit(1)
        fout.write(header)
        for line in fin:
            uid = line.strip("\n").split(",")[uid_index]
            if len(uids) < args.sample_size:
                uids.add(uid)
            if uid in uids:
                fout.write(line)
...
if __name__ == "__main__":
    ...
    p.add_argument("--fsum", type=float, help="total sum fraction to limit max.category number ", metavar="[0.683]", default=0.683)
    p.add_argument("--steps", type=int, help="forward steps for (long) target", metavar="[12]", default=12)
    # p.add_argument("--rolling", action="store_true", help="whether to generate rolling-window target sequence")
    ...
    p.add_argument("--sample_size", type=int, help="sample from raw CSV-data if size > 0", metavar="[0]", default=0)
    p.add_argument("--sample_id", type=str, help="column name to count sample", metavar=f"[{ID_COL}]", default=ID_COL)
    p.add_argument("--log_file", type=os.path.abspath, help="abspath to log file", metavar="[None]", default=None)
    args = p.parse_args()
