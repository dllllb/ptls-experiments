# rtd
checkpoints_folder="lightning_logs/rtd_model/version_1/checkpoints/*.ckpt"
output_file_prefix="data/rtd__"
conf_file="rtd_params"
batch_size=1024
...
rm results/epochs_rtd.txt
# rm -r conf/embeddings_validation.work/
python -m embeddings_validation \
    --config-dir conf --config-name embeddings_validation_short +workers=10 +total_cpu_count=20 \
    report_file="${hydra:runtime.cwd}/results/epochs_rtd.txt" \    
    auto_features=["${hydra:runtime.cwd}/data/rtd__???.pickle"]
