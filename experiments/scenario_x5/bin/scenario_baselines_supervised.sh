...
python -m ptls.pl_fit_target --conf conf/pl_fit_target_rnn.hocon
...
python -m ptls.pl_train_module --conf conf/mles_params_for_finetuning.hocon
# Fine tune the MeLES model in supervised mode and save scores to the file
python -m ptls.pl_fit_target --conf conf/pl_fit_finetuning_on_mles.hocon

# Fine tune the CPC model in supervised mode and save scores to the file
python -m ptls.pl_fit_target --conf conf/pl_fit_finetuning_on_cpc.hocon

# Train a special MeLES model for fine-tuning
# it is quite smaller, than one which is used for embeddings extraction, due to insufficiency labeled data to fine-tune a big model.
python -m ptls.pl_train_module --conf conf/rtd_params_for_finetuning.hocon
# Fine tune the RTD model in supervised mode and save scores to the file
python -m ptls.pl_fit_target --conf conf/pl_fit_finetuning_on_rtd.hocon
...
python -m ptls.pl_fit_target --conf conf/pl_fit_finetuning_on_barlow_twins.hocon
